class User < ApplicationRecord
    ...
    def get_feed_anime
        return Anime.select("*,(day_in_week+7-#{Time.now.getlocal.wday})%7 AS wday").where(id: self.followed_animes.pluck('id'), on_air: true).order("on_air DESC, wday, show_time")
    end
